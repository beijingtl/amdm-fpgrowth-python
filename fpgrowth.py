...
## FPNode class, nodes in the tree #############################################
    ...
    def add_actually(self, transaction, addcount = 1):
        ...
        for item in transaction:
            ...
            point = next_point
                            ...
                            # this is used when constructiong conditional trees
...
def main():
    ...
    if len(sys.argv) != 3:
        sys.exit('invalid amount of arguments, use python fptree.py filename.dat minsup')
    ...
    try:
        with open(data_file, 'r') as f: # since we use "with", no need to
                                        # close the file
...
#    for (freqset, count) in freq_tuples: # another output format
#        print('{' + ', '.join(freqset) + '}', '(' + str(count) + ')')
    
    
#    # mine, another output format
#    (freq_itemsets, freq_tuples) = fptree.mine_frequent_itemsets(minsupport)
#    # sort
#    freq_itemsets = rec_sort(freq_itemsets)
#    freq_itemsets = sorted(freq_itemsets, key=lambda lst: len(lst))
#    # output frequent itemsets
